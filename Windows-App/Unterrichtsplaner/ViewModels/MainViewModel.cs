    ...
    private void Laden()
    {
        try
        {
            if (File.Exists(_savePath))
            {
                ...
                _daten = JsonSerializer.Deserialize<Dictionary<string, PlanEintrag>>(json) ?? [];
            }
        }
        catch { _daten = []; }
    }
    ...
    private void Persistieren()
    {
        try
        {
            ...
            File.WriteAllText(_savePath,
                JsonSerializer.Serialize(_daten, new JsonSerializerOptions { WriteIndented = true }));
        }
        catch { }
    }
}
